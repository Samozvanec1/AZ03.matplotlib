...
input_file = 'prices.csv'  # входной файл с ценами
output_file = 'clean_prices.csv'  # выходной файл с очищенными ценами
...
with open(input_file, mode='r', encoding='utf-8') as infile, open(output_file, mode='w', newline='', encoding='utf-8') as outfile:
    ...
    for row in reader:  # обрабатываем остальные строки
        clean_row = row[:]  # Копируем всю строку, чтобы не потерять другие столбцы
        clean_row[0] = clean_price(row[0])  # очищаем цену с помощью функции clean_price и записываем в новую строку
...
price(f"Обработанные данные сохранены в {output_file}")